...
#  Usuario logado (variavel global) 
usuario_logado = None
...
def cadastrar_usuario():
    ...
    print("[OK] Usuario '" + nome + "' cadastrado com sucesso!")
...
def carregar_videos():
    """Retorna lista de dicionarios com os videos."""
    videos = []
...
def exibir_video(v):
    """Imprime as informacoes de um video formatado."""
    print("  ID      : " + v["id"])
    print("  Titulo  : " + v["titulo"])
    print("  Tipo    : " + v["tipo"])
    print("  Genero  : " + v["genero"])
    print("  Ano     : " + v["ano"])
...
def buscar_video():
    ...
    if len(encontrados) == 0:
        print("[AVISO] Nenhum video encontrado para '" + termo + "'.")
    else:
...
def listar_todos_videos():
    ...
    videos = carregar_videos()
    if len(videos) == 0:
        print("[AVISO] Nenhum video no catalogo.")
        return
    for v in videos:
        exibir_video(v)
...
def curtir_descurtir():
    print("\n CURTIR / DESCURTIR ")
    id_video = input("ID do video: ").strip()
    ...
    if ja_curtiu(usuario_logado, id_video):
        ...
        print("[OK] Voce descurtiu o video " + id_video + ".")
    else:
        adicionar_linha(ARQ_LIKES, usuario_logado + "|" + id_video)
        print("[OK] Voce curtiu o video " + id_video + "!")



#  PLAYLISTS
#  Formato no arquivo: usuario|nome_playlist|id1,id2,id3
#  Se nao tiver videos: usuario|nome_playlist|


def carregar_playlists_usuario(usuario):
    """Retorna lista de dicts das playlists do usuario logado."""
    playlists = []
...
def salvar_playlists_usuario(usuario, playlists):
    """Salva todas as playlists (mantendo as de outros usuarios)."""
    linhas = ler_linhas(ARQ_PLAYLISTS)
    # Remove linhas do usuario atual
    outras = []
    for linha in linhas:
        partes = linha.split("|")
        if partes[0] != usuario:
...
def listar_playlists():
    ...
    if len(playlists) == 0:
        print("  Voce nao tem playlists ainda.")
        return
...
def criar_playlist():
    ...
    for p in playlists:
        if p["nome"].lower() == nome.lower():
            print("[ERRO] Ja existe uma playlist com esse nome.")
            return
...
def editar_playlist():
    ...
    playlists = carregar_playlists_usuario(usuario_logado)
    if len(playlists) == 0:
        print("[AVISO] Voce nao tem playlists.")
        return

    listar_playlists()
    escolha = input("Numero da playlist: ").strip()
...
def excluir_playlist():
    ...
    listar_playlists()
    escolha = input("Numero da playlist para excluir: ").strip()
    ...
    confirma = input("Confirma exclusao de '" + nome + "'? (s/n): ").strip().lower()
    if confirma == "s":
        playlists.pop(idx)
        salvar_playlists_usuario(usuario_logado, playlists)
        print("[OK] Playlist '" + nome + "' excluida.")
    else:
        print("[AVISO] Operacao cancelada.")


def adicionar_video_playlist():
    print("\n ADICIONAR VIDEO A PLAYLIST")
    playlists = carregar_playlists_usuario(usuario_logado)
    if len(playlists) == 0:
        print("[AVISO] Voce nao tem playlists. Crie uma primeiro.")
        return
    ...
    id_video = input("ID do video a adicionar: ").strip()
    ...
    for v in videos:
        if v["id"] == id_video:
            ...
            break
    if not existe:
        print("[ERRO] Video nao encontrado no catalogo.")
        return

    if id_video in playlists[idx]["ids"]:
        print("[AVISO] Video ja esta nessa playlist.")
        return

    playlists[idx]["ids"].append(id_video)
    salvar_playlists_usuario(usuario_logado, playlists)
    print("[OK] Video " + id_video + " adicionado a playlist '" + playlists[idx]["nome"] + "'.")
...
def remover_video_playlist():
    ...
    p = playlists[idx]
    if len(p["ids"]) == 0:
        print("[AVISO] Essa playlist esta vazia.")
        return

    print("Videos na playlist:")
    for vid_id in p["ids"]:
        print("  - " + vid_id)

    id_video = input("ID do video a remover: ").strip()
    if id_video not in p["ids"]:
        print("[ERRO] Video nao encontrado nessa playlist.")
        return

    p["ids"].remove(id_video)
    salvar_playlists_usuario(usuario_logado, playlists)
    print("[OK] Video " + id_video + " removido da playlist.")
...
# DADOS
#  Popula o catalogo de videos se estiver vazio
...
def criar_dados_iniciais():
    if not os.path.exists(ARQ_VIDEOS) or os.path.getsize(ARQ_VIDEOS) == 0:
        ...
        print("[INFO] Catalogo de videos criado com 8 titulos.")
...
def menu_playlists():
    while True:
        ...
        print("5. Adicionar video a playlist")
        print("6. Remover video da playlist")
        print("0. Voltar")
...
def menu_logado():
    while True:
        print("\n FEItv ")
        print("Usuario: " + usuario_logado)
        print("1. Buscar video por nome")
        print("2. Listar todos os videos")
        print("3. Curtir / Descurtir video")
